...
class DeviceParameters:
    ...
    n_matrix_real: np.ndarray
    n_matrix_imag: np.ndarray
    omega_r: float
    g: float
    kappa: float
    resonator_dim: int
    ...
    def n_matrix(self) -> np.ndarray:
        return (self.n_matrix_real + ((1j) * (self.n_matrix_imag)))
    ...
    def validate(self) -> None:
        ...
        if self.n_matrix_real.shape != (self.fluxonium_dim, self.fluxonium_dim):
            raise ValueError("n_matrix_real must be square with size len(omega_levels).")
        if self.n_matrix_imag.shape != (self.fluxonium_dim, self.fluxonium_dim):
            raise ValueError("n_matrix_imag must be square with size len(omega_levels).")
        if self.n_matrix_imag.shape != self.n_matrix_imag.shape:
            raise ValueError("n_matrix_real must be equal in shape to n_matrix_imag")  
...
def load_json(path: str | Path) -> tuple[DeviceParameters, PulseParameters, SimulationParameters]:
    ...
    params = DeviceParameters(
        ...
        n_matrix_real=np.asarray(device["n_matrix_real"], dtype=float),
        n_matrix_imag=np.asarray(device["n_matrix_imag"], dtype=float),
